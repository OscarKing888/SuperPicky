...
class PipelineBuilder:
    ...
    def build_unified_ai_processing_pipeline(
        ...
        for device_config in device_configs:
            ...
            print(f"====[DBG] device: {device}, max_workers: {max_workers}")
        ...
        if not has_cpu_stage and preprocess_workers > 0:
            ...
            print(f"====[DBG] preprocess_stage: {preprocess_stage}")
...
class ParallelPipelineBuilder(PipelineBuilder):
    """Job queue builder that shares CPU/GPU pools and queues."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._heif_convert_queue: Optional[JobQueue] = None
        self._shared_ai_queue: Optional[JobQueue] = None

    def _get_or_create_heif_queue(self) -> JobQueue:
        if self._heif_convert_queue is None:
            self._heif_convert_queue = JobQueue()
        return self._heif_convert_queue

    def _get_or_create_shared_ai_queue(self, queue: Optional[JobQueue]) -> JobQueue:
        if self._shared_ai_queue is None:
            self._shared_ai_queue = queue or JobQueue()
        elif queue is not None and queue is not self._shared_ai_queue:
            # Preserve the existing shared queue to avoid splitting job streams.
            pass
        return self._shared_ai_queue
    ...
    def build_heif_conversion_stage(
        ...
        """
        Enqueue HEIF conversion jobs into the shared CPU conversion queue.
        Actual conversion is handled by the unified CPU pool.
        """
        heif_input_queue = self._get_or_create_heif_queue()
        self._shared_ai_queue = output_queue
        ...
        for filename, filepath in heif_files:
            ...
            job = Job(
                ...
                data={
                    ...
                    'filepath': filepath,
                    'file_prefix': file_prefix,
                    'job_type': JOB_TYPE_HEIF_CONVERT,
                },
            )
            heif_input_queue.put(job)

        monitor_stage = QueueMonitorStage(
            name="HEIF-Queue",
            input_queue=heif_input_queue,
            log_callback=self.log_callback,
        )
        return Pipeline([monitor_stage], log_callback=self.log_callback)
    ...
    def build_unified_ai_processing_pipeline(
        ...
        inference_queue = self._get_or_create_shared_ai_queue(shared_ai_queue)
        heif_convert_queue = self._get_or_create_heif_queue()
        ...
        if cpu_max_workers_override is not None:
            cpu_threads = cpu_max_workers_override
        cpu_threads = max(1, cpu_threads)

        conversion_workers = min(4, max(1, cpu_threads - 1))

        cpu_stage = CPUHybridStage(
            convert_queue=heif_convert_queue,
        ...
        for device_config in device_configs:
            ...
            gpu_stages.append(ai_stage)

        stages = [cpu_stage] + gpu_stages + [exif_stage]
        pipeline = Pipeline(stages, log_callback=self.log_callback)
